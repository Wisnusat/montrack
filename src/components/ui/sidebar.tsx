...
import { NotebookPen, CreditCard, FileBarChart, LayoutDashboard, Settings, LogOut } from 'lucide-react'
    ...
    return (
        <aside className="p-6 flex flex-col gap-8 bg-zinc-900 text-white h-screen">
            <div className="flex flex-col items-center gap-2">
                <Avatar>
                    <AvatarImage src="https://github.com/shadcn.png" className="rounded-full" style={{ width: '68px', height: '68px' }} />
                    <AvatarFallback>FY</AvatarFallback>
                </Avatar>
                <h2 className="font-medium">Fiko Yorisdwi &apos;Aliy</h2>
            </div>
                    ...
                    {sidebarItems.map((item) => (
                        <Link to={item.path}>
                            <Button
                                key={item.label}
                                variant={"ghost"}
                                className={`${isActive(item.path) && "bg-[#48DE80] text-black"} justify-start gap-4 w-full flex items-center`}
                            >
                                <span>{item.icon}</span>
                                {item.label}
                            </Button>
